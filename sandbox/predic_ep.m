...
function [yp,erp] = predic_ep(y,u,A,B,C,D,K,ax)
...
erv = (y(1:end-1,:)-yp(2:end,:));
erp = sqrt(sum(erv.^2)./sum(y(1:end-1,:).^2))*100;

idx = find((erp > 100) | isnan(erp));
erp(idx) = ones(size(idx))*100;
