...
m = 2;l = 2;n = 4;
% a1 = [0.1*ones(floor(N/2),1);0.1*ones(ceil(N/2),1)]; % constant
% a2 = [0.2*ones(floor(N/2),1);0.2*ones(ceil(N/2),1)]; % constant
% a3 = [0.7*ones(floor(N/2),1);0.7*ones(ceil(N/2),1)]; % constant
% a4 = [0.8*ones(floor(N/2),1);0.8*ones(ceil(N/2),1)]; % constant
% a1 = 0.125+0.1*gensig('sin',N/80,(N-1)*0.1,0.1); % sin wave
% a2 = 0.375+0.1*gensig('sin',N/80,(N-1)*0.1,0.1); % sin wave
% a3 = 0.625+0.1*gensig('sin',N/80,(N-1)*0.1,0.1); % sin wave
% a4 = 0.875+0.1*gensig('sin',N/80,(N-1)*0.1,0.1); % sin wave
% a1 = 0.125+0.1*gensig('square',N/80,(N-1)*0.1,0.1); % square wave
% a2 = 0.375+0.1*gensig('square',N/80,(N-1)*0.1,0.1); % square wave
% a3 = 0.625+0.1*gensig('square',N/80,(N-1)*0.1,0.1); % square wave
% a4 = 0.65+0.1*gensig('square',N/80,(N-1)*0.1,0.1); % square wave
t = linspace(0,1,N); t = t'; 
a1 = 0.125+0.1*sin(2*pi*10*t+randn(1));
a2 = 0.375+0.3*sin(2*pi*20*t+randn(1));
a3 = 0.625+0.3*sin(2*pi*30*t+randn(1));
a4 = 0.875+0.1*sin(2*pi*10*t+randn(1));
% b = randn(4,2); c = randn(2,4);
